...
const navCartBadge = document.querySelector(".badge");
  ...
  try {
    fetch("/cart/items", {
      ...
      .then((data) => {
        ...
        const cartItemTotalPriceElement = event.target.parentElement.querySelector(".cart-item-total-price");
        cartItemTotalPriceElement.textContent = data.updatedCartData.updatedItemTotalPrice.toFixed(2);
        cartTotalPriceElement.textContent = data.updatedCartData.newCartTotalPrice.toFixed(2);
        navCartBadge.textContent = data.updatedCartData.newTotalItems;
      });
  } catch (error) {
